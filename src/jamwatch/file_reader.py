...
class FileReader(ABC):
    ...
    def get_files_list(
        self, randomize: bool = True, verbose: bool = False
    ) -> list[File]:
        ...
        for records in files:
            for record in records:
                if not isinstance(record, dict) or record == {}:
                    continue
                file_list = list(
                    itertools.chain(r for r in record.values() if isinstance(r, dict))
                )
                for i, _ in enumerate(file_list):
                    file: File = _
                    if (
                        file.get("type", None) != "file"
                        and Path(file.get("name", "")).suffix.lower() != ".mp3"
                    ):
                        continue
                    if verbose:
                        logger.info(
                            f"[{current_file_count}] Get track details for {file['name']}"
                        )
...
class FtpFileReader(FileReader):
    def __init__(
        self, host: str, username: str, password: str, path: str, port: int = 21
    ) -> None:
        self.fs = fsspec.filesystem(
            "ftp", host=host, username=username, password=password, port=port
        )
        self.path = path
